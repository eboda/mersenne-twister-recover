...
class MT19937Recover:
    ...
    def untemper(self, v):
        """ Reverses the tempering which is applied to each output of MT19937 """
    ...
    def go(self, outputs, forward=True):
        ...
        Args:
            outputs (List[int]): list of at least 624 observed outputs from the PRNG.
                However, at least 625 outputs are required to correctly recover the internal index.
            forward (bool): Forward internal state until all observed outputs are generated.
        ...
        if len(outputs) >= 625:
            # We have additional outputs and can correctly recover the internal index by bruteforce
            ...
            # With only 624 outputs we assume they were the first observed 624 outputs after
            # a twist. Therefore we set the internal index to 624.
